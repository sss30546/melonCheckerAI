...
st.set_page_config(page_title="🍋 Lemon Disease Analysis Chatbot", page_icon="🍋", layout="wide")
...
1. **โรคแคงเกอร์ (Citrus Canker)**  
   - ใบอ่อนมีจุดนูนสีน้ำตาล ล้อมด้วยวงสีเหลือง 
   - ผลมีแผลสีน้ำตาลอยู่บางบริเวณ
   - แผลแข็งและหยาบคล้ายแผลเน่า เมื่อลูบจะสากมือ  
   - สาเหตุ: แบคทีเรีย *Xanthomonas citri*  
   - แนวทางจัดการ: ตัดกิ่งที่เป็นโรค, ใช้สารคอปเปอร์, สร้างแนวกันลมเพื่อลดการแพร่กระจายของเชื้อโรค

2. **โรคสแคบ (Citrus Scab)**  
   - ใบและผลอ่อนมีตุ่มนูนคล้ายสะเก็ดทั่วบริเวณ 
...
3. **โรคแอนแทรคโนส (Anthracnose)**  
   - ผลเลม่อนพบจุดสีน้ำตาลหรือดำ แผลบริเวณผลแห้ง 
   - ผลร่วงง่าย 
   - ใบร่วงง่าย
   - ผลผิดรูป รูปร่างผิดปกติ รูปร่างไม่แน่นอน
   - กิ่งก้านแห้งตายและมีการปกคลุมไปด้วยผงสปอร์สีดำ
   - ใบร่วงก่อนกำหนดและใบมีการปกคลุมไปด้วยสปอร์สีดำ
...
def main():
    ...
    st.title("🍋 Lemon Disease Analysis Chatbot")
    st.subheader("บรรยายลักษณะของเลมอน แล้วให้ AI วิเคราะห์ว่าอาจเป็นโรคอะไร")

    if "messages" not in st.session_state:
        st.session_state["messages"] = [
            {"role": "assistant", "content": (
                "สวัสดีจ้า 🍋 ฉันชื่อน้องสตางค์ ฉันเป็นแชตบอตผู้ช่วยวิเคราะห์โรคในเลมอน\n"
                "ฉันสามารถวิเคราะห์ได้ทั้งหมด 3 โรค ได้แก่ Citrus canker, Citrus scab, Anthracnose \n\n"
                "โปรดบอกอาการของเลมอนทั้งหมด 3 ด้าน ได้แก่ ลักษณะของผล ใบ และกิ่ง/ยอด เพื่อให้ฉันวิเคราะห์ได้อย่างแม่นยำมากที่สุด \n"
                "⚠️ โปรดบอกอาการ เช่น:\n"
                "- ใบมีแผลเป็นวงกลมสีน้ำตาลเข้ม แผลนูนรอบแผลมีขอบสีเหลือง ใบบางใบเริ่มเหลืองและร่วง ผลมีจุดแผลนูนแข็งหลายจุดสีเข้ม  กิ่งอ่อนมีแผลเล็ก ๆ กระจาย\n"
                "- ผิวมีแผลสีขาวเป็นสะเก็ดขนาดกว้าง ใบมีจุดนูนเล็กๆ ใบแข็งมาก ผลเลม่อนผิดรูป\n"
                "- ใบมีจุดสีน้ำตาลเข้มทั่วบริเวณ ผลมีส่วนเน่าเล็กน้อยและรูปร่างผิดปกติ\n\n"
                "เมื่อบอกอาการให้ฉันทราบแล้วให้ผู้ใช้กดปุ่ม 🚀 ส่งข้อความ จากนั้นฉันจะช่วยวิเคราะห์ให้ทันที❗️"
            )}
        ]

    with st.form("chat_form"):
        query = st.text_input("🗣️ คุณ:", placeholder="เช่น 'ใบมีจุดนูนสีน้ำตาล ขอบใบเหลืองรอบๆแผล ผลมีแผลเน่า'").strip()
    ...
    st.markdown("---")
    st.caption("🌱 พัฒนาโดยทีม Future Lab โรงเรียนสตรีศรีน่าน | 🍋 ระบบแชตบอตวิเคราะห์โรคในเลมอนเบื้องต้น (Lemon Disease Analysis Chatbot) - version : 1.3.6")
